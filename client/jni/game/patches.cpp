...
void ApplyPatches()
{
    ...
    // new patches
   // ------------------------------------------------------------------------------------------------------------     
	makeNOP(g_libGTASA + 0x0056CD4A, 2); // 0x56CD4A

	//                   (gta 1.08)
	WriteMemory(g_libGTASA + 0x005197E0, (uintptr_t)"\x04", 1); // 0x5197E0
	WriteMemory(g_libGTASA + 0x005197EA, (uintptr_t)"\xFA", 1); // 0x5197EA

    // CPopulation::AddPed(ePedType,uint,CVector const&,bool)	0x45F1A4	
	WriteMemory(g_libGTASA + 0x0045F1A4, (uintptr_t)"\xF7\x46", 2);
	...
	WriteMemory(g_libGTASA + 0x00443EF4, (uintptr_t)"\xF7\x46", 2); // CPedGroupDefaultTaskAllocatorRandom::AllocateDefaultTasks | crash

                  // CVehicleRecording::Load(RwStream *,int,int)	0x2DC8E0	
	// WriteMemory(g_libGTASA + 0x002DC8E0); // CVehicleRecording::Load
	// WriteMemory(g_libGTASA + 0x2DC8E0, (uintptr_t)"\xF7\x46", 2, (uintptr_t)"\xF7\x46", 2);
                  // CPedGroups::Process(void)	0x448B38	
	WriteMemory(g_libGTASA + 0x00448B38, (uintptr_t)"\xF7\x46", 2); // CPedGroups::Process() | crash

                  // CPedGroupIntelligence::Process(void)	0x448850	
	WriteMemory(g_libGTASA + 0x00448850, (uintptr_t)"\xF7\x46", 2); // CPedGroupIntelligence::Process | crash

                  // CPedGroupMembership::SetLeader(CPed *)	0x444B18	
	WriteMemory(g_libGTASA + 0x00444B18, (uintptr_t)"\xF7\x46", 2); // CPedGroupMembership::SetLeader | crash

                  // CPedGroupIntelligence::Flush(void)	0x444FDC	
	WriteMemory(g_libGTASA + 0x004B3156, (uintptr_t)"\xF7\x46", 2); // CPedGroupIntelligence::Flush | crash

                  // WaterCreatureManager_c::Update(float)	0x51DEC4	
	WriteMemory(g_libGTASA + 0x0051DEC4, (uintptr_t)"\xF7\x46", 2); //                
 

    // CMotionBlurStreaks::Update	
    makeNOP(g_libGTASA + 0x0054D720, 2); 

	// CCamera::RenderMotionBlur(void)
   	makeNOP(g_libGTASA + 0x0039B394, 2);

    // RenderQueue::RenderQueue(void)	0x1A7EB0	
	WriteMemory(g_libGTASA + 0x001A7EB0, (uintptr_t)"\x4F\xF4\x00\x10\x4F\xF4\x80\x06", 8); 	// RenderQueue::RenderQueue
	//CHook::WriteMemory(g_libGTASA + 0x001D193A, "\x4F\xF4\x00\x16", 4); 	// RenderQueue::RenderQueue

    // CPlayerPed::AnnoyPlayerPed(bool)	0x457644	
    WriteMemory(g_libGTASA + 0x00457644, (uintptr_t)"\xF7\x46", 2); //        // CPlayerPed::AnnoyPlayerPed | N                  
