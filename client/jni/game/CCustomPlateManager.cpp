...
{
	char path[0xFF];
	sprintf(path, OBFUSCATE("%sSAMP/plates/ru_font.ttf"), g_pszStorage);

	m_pRUFont = CFontRenderer::AddFont(path, 128);
	m_pKZFont = CFontRenderer::AddFont(path, 132);

	sprintf(path, OBFUSCATE("%sSAMP/plates/ua_font.ttf"), g_pszStorage);
	...
	m_pUaSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB(OBFUSCATE("samp"), OBFUSCATE("plate_ua"));
	m_pRuSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB(OBFUSCATE("samp"), OBFUSCATE("plate_ru"));
	m_pBuSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB(OBFUSCATE("samp"), OBFUSCATE("plate_bu"));
	m_pKzSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB(OBFUSCATE("samp"), OBFUSCATE("plate_kz"));
	m_pRuPoliceSprite->m_pRwTexture = (RwTexture*)LoadTextureFromDB(OBFUSCATE("samp"), OBFUSCATE("plate_ru_police"));
	...
	for (auto& i : m_Plates)
	{
		...
		if (pPlate)
		{
			if (pNetGame->GetVehiclePool())
			{
				...
				{
					pNetGame->GetVehiclePool()->GetAt(i.vehicleId)->ApplyTexture(OBFUSCATE("remap_plate"), pPlate);
				}
			}
		}
	}
