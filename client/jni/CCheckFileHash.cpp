...
bool CCheckFileHash::IsFilesValid()
{
	...
	auto* mdCheckSumFile = new SMDFile;

	for (const auto & g_aHashedFile : g_aHashedFiles)
	{
		sprintf(mdCheckSumFile->szFileLocation, g_aHashedFile.encStr.decrypt(), g_pszStorage);

		mdCheckSumFile->iCorrectDigestArray[0] = g_aHashedFile.uiCorrectDigest1;
		mdCheckSumFile->iCorrectDigestArray[1] = g_aHashedFile.uiCorrectDigest2;
		mdCheckSumFile->iCorrectDigestArray[2] = g_aHashedFile.uiCorrectDigest3;
		mdCheckSumFile->iCorrectDigestArray[3] = g_aHashedFile.uiCorrectDigest4;
