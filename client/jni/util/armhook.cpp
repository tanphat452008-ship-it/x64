...
void installHook(uintptr_t addr, uintptr_t func, uintptr_t *orig)
{
	...
	Log("memlib_end: %X, memlib_start: %X, mmap_end: %X, mmap_start: %x", memlib_end, memlib_start, mmap_end, mmap_start);

    if(memlib_end < (memlib_start + 0x10) || mmap_end < (mmap_start + 0x20))
    {
    	Log("memlib_Start + 0x10: %X, mmap_start + 0x20: %X", memlib_start + 0x10, mmap_start + 0x20);
