...
void InitBASSFuncs()
{
	...
	handle = dlopen("/data/data/com.samp.online/lib/libbass.so", 3);
#else
	handle = dlopen("/data/data/com.samp.online/lib/libbass.so", 3);
#endif
	if (!handle)
...
bool ProcessCommands(const char* str)
{
	...
	else if (strcmp(str, "/save") == 0)
	{
		...
		if(pPlayer->IsInVehicle())
		{
			...
			fprintf(fileOut, "AddStaticVehicle(%u,%.4f,%.4f,%.4f,%.4f);\n",
		...
		else
		{
			...
			fprintf(fileOut, "AddPlayerClass(%u,%.4f,%.4f,%.4f,%.4f,0,0,0,0,0,0);\n", 
