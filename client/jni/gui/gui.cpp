...
extern CGame* pGame;
extern CButtonPanelEx *pButtonPanelEx;
...
void CGUI::Render()
{
	...
	if (pCrossHair)		pCrossHair->Render();
	if (pKeyBoard)		pKeyBoard->ProcessInputCommands();
	if (pPlayerTags) 	pPlayerTags->Render();
	if (pNetGame && pNetGame->GetLabelPool()) pNetGame->GetLabelPool()->Draw();
    if (pKillList) 		pKillList->Render();
	...
	if (pButtonPanelEx) pButtonPanelEx->Render();
	...
	if (pNetGame && !pDialogWindow->m_bIsActive && pGame->IsToggledHUDElement(HUD_ELEMENT_BUTTONS) && !pSettings->GetReadOnly().iHud)
	{
	ImGuiIO& io = ImGui::GetIO();
...
void CGUI::RenderVersion()
{
	...
	RenderText(pos, ImColor(255, 255, 255), true, "1.08x64", nullptr, 20);
}
