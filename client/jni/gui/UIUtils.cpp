...
ImColor RenderTextAndGetLastColor(ImFont *font, const float font_size, uint8_t outline, ImVec2 pos, ImColor col, const char* szStr)
{
    ...
    while(textCur < (tempStr + sizeof(tempStr)) && *textCur != '\0')
    {
        if(*textCur == '{')
        {
            if(textCur != textStart)
            {
                pGUI->RenderMaterialText(vecPos, col, outline, textStart, textCur, font_size, font);
                vecPos.x += ImGui::CalcTextSize(font, font_size, textStart, textCur).x;
            }
...
ImVec2 CalcTextSizeWithoutTags(ImFont *font, const float font_size, const char* szStr)
{
    ...
    szNonColored[iNonColoredMsgLen] = 0;
    return ImGui::CalcTextSize(font, font_size, szNonColored);
}
