...
void CDebugInfo::Draw()
{
    ...
    {     
		...
		snprintf(szStr, 30, "FPS: %.1f", *pFPS);
		pos = ImVec2(pGUI->ScaleX(10.0f), pGUI->ScaleY(580.0f - pGUI->GetFontSize()));
	...
	if (m_uiDrawDebug || m_uiDrawFPS)
    {
		...
		sprintf(szStr2, "MEM: %.1f/%.1f (%.1f %%)", (float)msUsed/ (1024*1024), (float)msAvailable / (1024*1024), percentUsed);
		pos2 = ImVec2(pGUI->ScaleX(10.0f), pGUI->ScaleY(605.0f - pGUI->GetFontSize()));
        pGUI->RenderText(pos2, (ImU32)0xFFFFFFFF, true, &szStr2[0]);
	}
    ...
    {     
        ...
        if(pPlayerPed) 
        {
            if(!pPlayerPed->IsInVehicle()) {
                sprintf(szStr3,"OnFoot Send Rate: %u\n", pLocalPlayer->GetOptimumOnFootSendRate());
                pos3 = ImVec2(pGUI->ScaleX(10.0f), pGUI->ScaleY(630.0f - pGUI->GetFontSize()));

                pGUI->RenderText(pos3, (ImU32)0xFFFFFFFF, true, &szStr3[0]);
            } else {
                sprintf(szStr3,"InCar Send Rate: %u\n", pLocalPlayer->GetOptimumInCarSendRate());
                pos3 = ImVec2(pGUI->ScaleX(10.0f), pGUI->ScaleY(630.0f - pGUI->GetFontSize()));
