...
CButtonPanelEx *pButtonPanelEx = nullptr;
...
void InitInMenu()
{
	...
	pButtonPanelEx = new CButtonPanelEx();
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	makeNOP(g_libGTASA + 0x0039844E, 2);
	makeNOP(g_libGTASA + 0x0039845E, 2);
	makeNOP(g_libGTASA + 0x0039840A, 2);

	makeNOP(g_libGTASA + 0x00398972, 2); // get out fucking roadblocks
...
{	
	...
	{
		sprintf(buffer, OBFUSCATE("%slogs//samp_log.txt"), g_pszStorage);
...
void LogVoice(const char *fmt, ...)
{
	...
	if(flLog == nullptr && g_pszStorage != nullptr)
	{
		sprintf(buffer, OBFUSCATE("%slogs/%s"), g_pszStorage, SV::kLogFileName);
...
void CrashLog(const char* fmt, ...)
{
	...
	if (flLog == nullptr && g_pszStorage != nullptr)
	{
		sprintf(buffer, OBFUSCATE("%slogs/crash_log.log"), g_pszStorage);
