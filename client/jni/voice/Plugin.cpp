...
void Plugin::MainLoop()
{
    ...
    while(const auto voicePacket = Network::ReceiveVoicePacket())
    {
        ...
        LogVoice("[dbg:samp:load] : LOAD TWO");
    }
    ...
    //int defaultKey = pSettings->GetReadOnly().iVoiceChatKey;
    int defaultKey = 0x42;
    if(KeyFilter::IsKeyAvailable(defaultKey) &&
        MicroIcon::IsShowed() /*&& pSettings->GetReadOnly().bVoiceChatEnable*/)
    {
        ...
        ImVec2 vecButSize = ImVec2(pSettings->GetReadOnly().fButtonMicrophoneSize * 5 + 5.0f, pSettings->GetReadOnly().fButtonMicrophoneSize * 5);
        ...
        ImGui::SetWindowPos(ImVec2(pGUI->ScaleX(pSettings->GetReadOnly().fButtonMicrophoneX), pGUI->ScaleY(pSettings->GetReadOnly().fButtonMicrophoneY)));
        ImGui::End();
