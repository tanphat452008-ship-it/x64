...
import com.samp.online.gui.BrNotification;
import com.samp.online.gui.Donate;
import com.samp.online.gui.Speedometer;
import com.samp.online.gui.dialogs.Dialog;
import com.samp.online.core.DialogClientSettings;
...
private Runnable hideNotificationRunnable = null;
    ...
    public native void sendButton(int type, byte[] str, int action);
...
public void surfaceChanged(SurfaceHolder holder, int format, int width, int height)
{
    ...
    // 1. Khai báo biến này ở đầu Class (bên ngoài hàm setPauseState)
private boolean mWasCefVisibleBeforePause = false;

// 2. Sửa lại hàm setPauseState
public void setPauseState(boolean z2) {
    ...
    runOnUiThread(() -> {
        if (z2) { 
            if (mWasCefVisibleBeforePause) {
                mJavaManager.hideBrowserView();
            }
            
            mAndroidUI.setVisibility(View.GONE);
            ...
            mAndroidUI.setVisibility(View.VISIBLE);
        }
    });
}
...
public void showBrNotification(final String text, final int type, final int duration) {
    ...
    public void showNotification(int delay, String name, String msg, String url) {
        JSONObject jSONObject = new JSONObject();
        try {
            jSONObject.put("n", name);
            jSONObject.put("m", msg);
            jSONObject.put("u", url);
            jSONObject.put("d", delay);
